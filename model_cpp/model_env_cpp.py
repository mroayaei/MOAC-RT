...
class CellEnvironment(Environment):
    ...
    def reset(self, mode):
        ...
        self.controller_capsule = cppCellModel.controller_constructor(50, 50, 100, 350)
        self.init_hcell_count = cppCellModel.HCellCount()
        self.init_ccell_count = cppCellModel.CCellCount()
        if mode == -1:
            self.verbose = False
        else :
            self.verbose = True
    ...
    def act(self, action):
        ...
        self.rest_c_gained += (post_ccell - p_ccell)
        reward = self.adjust_reward(dose, rest, pre_ccell, p_ccell, post_ccell, pre_hcell, p_hcell, post_hcell)
        if self.verbose:
                # print("Radiation dose :", dose, "Gy ",  "ckilled=",pre_ccell-p_ccell , "remainingc :", post_ccell, "hkilled=",pre_hcell-p_hcell ,"sumhk=",self.radiation_h_killed,  "time =", rest, "reward=", reward)
                print("Dose : {:2.1f} Gy * preC : {:4} * KC : {:4} * remC : {:4} * preH : {:4} * KH : {:4} * sumKH : {:4} * Rest : {:2} * Reward : {}".format(dose,pre_ccell,pre_ccell-post_ccell,post_ccell,pre_hcell,pre_hcell-p_hcell,self.radiation_h_killed,rest,reward))
        return reward, dose, rest, pre_hcell-p_hcell

    def surviving_fraction(self):
        return cppCellModel.HCellCount() / self.init_hcell_count

    def adjust_reward(self, dose,rest, pre_ccell, p_ccell, post_ccell, pre_hcell, p_hcell, post_hcell):

        ccell_killed = pre_ccell - post_ccell
        hcells_lost = pre_hcell-min(post_hcell, p_hcell)
        
        if self.special_reward and self.inTerminalState()[0]:
            if self.end_type == "L" or self.end_type == "T":
                return -1
               
            else:
                if self.reward == 'dose':
                    return 0.5 - (self.init_hcell_count - cppCellModel.HCellCount()) / 3000 
    ...
    def inTerminalState(self):
        if cppCellModel.CCellCount() <= 0 :
            if self.verbose:
                print("Cancer wins")        
            self.end_type = 'W'
            return True, self.end_type
        elif cppCellModel.HCellCount() < 10:
            if self.verbose:
                print("Cancer wins")
            self.end_type = "L"
            return True, self.end_type
        elif cppCellModel.controllerTick(self.controller_capsule) > 1200:
            if self.verbose:
                print("Time out!")
            self.end_type = "T"
            return True, self.end_type
